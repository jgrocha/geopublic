...
if (ServerConfig.https) {
    ssloptions = {
        key: fs.readFileSync(ServerConfig.key, 'utf8'),
        cert: fs.readFileSync(ServerConfig.cert, 'utf8')
    };
}
...
app.get(global.App.prefix + '/registo/:lang/:id', routes.registo(global.App.database));
app.get(global.App.prefix + '/reset/:lang/:id', routes.reset(global.App.database));
...
if (ServerConfig.https) {
    var servidor = https.createServer(ssloptions, app).listen(ServerConfig.porthttps, function () {
        console.log("Express server listening on port %d in %s mode", ServerConfig.porthttps, app.settings.env);
    });
}
