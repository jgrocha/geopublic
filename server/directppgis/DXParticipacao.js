...
var DXParticipacao = {
    ...
    readPlano: function (params, callback, sessionID, request) {
        ...
        var sql = 'SELECT id, idpromotor, designacao, descricao, responsavel, email, site, inicio, fim, datamodificacao, proposta, idutilizador, ST_AsGeoJSON(the_geom) as the_geom, alternativeproposta, active, planocls, background, fim < now() as closed, logo FROM ppgis.plano ' + where + ' ORDER BY fim DESC';
        console.log(sql);
        conn.query(sql, function (err, result) {
            if (err) {
                console.log('SQL=' + sql + ' Error: ', err);
